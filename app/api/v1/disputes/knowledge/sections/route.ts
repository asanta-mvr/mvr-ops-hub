...
import { listSections, createSection } from '@/lib/disputes/sections'
...
export async function GET() {
  try {
    ...
    return NextResponse.json({ data: await listSections() })
  } catch (error) {
    console.error('[GET /api/v1/disputes/knowledge/sections]', error)
...
export async function POST(req: NextRequest) {
  try {
    ...
    const section = await createSection(parsed.data.label, session!.user.id)
    ...
    db.auditLog
      .create({
        data: {
          ...
          recordId: section.id,
          newData: JSON.parse(
            JSON.stringify({ label: section.label })
          ) as Prisma.InputJsonValue,
          ipAddress: req.headers.get('x-forwarded-for') ?? req.headers.get('x-real-ip') ?? undefined,
          userAgent: req.headers.get('user-agent') ?? undefined,
        },
      })
      .catch((e) => console.error('[audit] dispute_knowledge_sections CREATE', e))

    return NextResponse.json({ data: section }, { status: 201 })
  } catch (error) {
    console.error('[POST /api/v1/disputes/knowledge/sections]', error)
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 })
  }
}
