...
function parseToBlocks(text: string | null | undefined, defaultSection: 'rules' | 'kb' = 'kb'): ContentBlock[] {
  if (!text) return []
  try {
...
function SortableBlock({
  ...
  block: ContentBlock; onEdit: (b: ContentBlock) => void; onDelete: (id: string) => void
}) {
  ...
  return (
    <div
      ref={setNodeRef}
      style={{ transform: CSS.Transform.toString(transform), transition, opacity: isDragging ? 0.4 : 1, zIndex: isDragging ? 10 : undefined, position: 'relative' }}
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-sm text-muted-foreground leading-relaxed whitespace-pre-wrap break-words">{block.description}</p>
      </div>
      <div className="flex items-center gap-1 mt-0.5 shrink-0 opacity-0 group-hover:opacity-100 transition-opacity">
        <button onClick={() => onEdit(block)} className="text-muted-foreground hover:text-mvr-primary transition-colors" aria-label="Edit block">
          <Pencil className="w-3.5 h-3.5" />
        </button>
        <button onClick={() => onDelete(block.id)} className="text-muted-foreground hover:text-mvr-danger transition-colors" aria-label="Delete block">
...
interface HouseRulesFormEditorProps {
  value:        string | undefined
  onChange:     (value: string) => void
  buildingName: string
}

export default function HouseRulesFormEditor({ value, onChange, buildingName }: HouseRulesFormEditorProps) {
  const [activeTab, setActiveTab] = useState<Tab>('rules')

  const allBlocks = parseToBlocks(value)
  ...
  const filteredBlocks = useMemo(() => {
    ...
    return blocks.filter(b => b.title.toLowerCase().includes(q) || b.description.toLowerCase().includes(q))
  }, [blocks, search])

  const emit = useCallback((rules: ContentBlock[], kb: ContentBlock[]) => {
    const combined = [
  ...
  function openAdd() {
    setEditingId(null); setDraftTitle(''); setDraftDesc(''); setModalOpen(true)
  }

  function openEdit(block: ContentBlock) {
    setEditingId(block.id); setDraftTitle(block.title); setDraftDesc(block.description); setModalOpen(true)
  }

  function handleSave() {
    if (!draftTitle.trim()) return
    if (editingId !== null) {
      mutate(blocks.map(b => b.id === editingId ? { ...b, title: draftTitle.trim(), description: draftDesc.trim() } : b))
    } else {
      mutate([...blocks, { id: genId(), title: draftTitle.trim(), description: draftDesc.trim(), section: activeTab === 'rules' ? 'rules' : 'kb' }])
      setSearch('')
    }
    setModalOpen(false)
  }
  ...
  async function handleDownload() {
    setDownloading(true)
    try { await downloadPdf(rulesBlocks, kbBlocks, buildingName) }
  ...
  return (
    <>
      <div className="rounded-xl border border-[#E0DBD4] overflow-hidden bg-mvr-cream/40">
        {/* Tabs + actions */}
        <div className="flex items-stretch border-b border-[#E0DBD4]">
            ...
            return (
              ...
              >
                ...
                <span className="ml-1.5 text-[10px] font-normal opacity-60">
                  ({tab === 'rules' ? rulesBlocks.length : kbBlocks.length})
                </span>
              </button>
            )
          })}
          ...
          <div className="flex items-center gap-1.5 px-3 border-l border-[#E0DBD4] shrink-0">
            ...
            <button
              ...
              title={`Agregar bloque — ${tabLabel(activeTab)}`}
            >
        ...
        {/* Search */}
        <div className="px-4 py-3 border-b border-[#E0DBD4] bg-white">
          <div className="relative">
            ...
            <input
              ...
              placeholder="Buscar…"
              className="w-full pl-8 pr-3 py-1.5 text-sm border border-[#E0DBD4] rounded-lg focus:outline-none focus:ring-2 focus:ring-mvr-primary/20 focus:border-mvr-primary bg-white"
            />
          </div>
        </div>

        {/* Blocks list */}
        <div className="p-4 space-y-2 min-h-[120px] bg-white">
          {filteredBlocks.length === 0 ? (
            <div className="py-8 flex flex-col items-center gap-2 text-center">
              <p className="text-sm text-muted-foreground">
                {search.trim() ? 'Sin coincidencias.' : 'Sin bloques aún.'}
              </p>
              {!search.trim() && (
                <button
                  type="button"
            ...
            filteredBlocks.map((block) => (
              <StaticBlock key={block.id} block={block} onEdit={openEdit} onDelete={handleDelete} />
            ))
          ) : (
            <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
              <SortableContext items={filteredBlocks.map(b => b.id)} strategy={verticalListSortingStrategy}>
                {filteredBlocks.map((block) => (
                  <SortableBlock key={block.id} block={block} onEdit={openEdit} onDelete={handleDelete} />
                ))}
              </SortableContext>
            </DndContext>
          )}
          ...
          <div className="w-full max-w-xl bg-white rounded-2xl shadow-xl border overflow-hidden">
            <div className="flex items-center justify-between px-5 py-4 border-b border-[#E0DBD4]">
              <h3 className="text-sm font-semibold text-mvr-primary">
                {isEditing
                  ? `Editar bloque — ${tabLabel(activeTab)}`
                  : activeTab === 'rules' ? 'Agregar House Rule' : 'Agregar Knowledge Base Entry'}
              </h3>
              <button
                type="button"
            ...
            <div className="px-6 py-5 space-y-5">
              <div>
                <label className="block text-xs font-medium text-mvr-primary mb-1.5">
                  Título <span className="text-mvr-danger">*</span>
                </label>
                <input
                  autoFocus
                  type="text"
              ...
              <div>
                <label className="block text-xs font-medium text-mvr-primary mb-1.5">Descripción</label>
                <textarea
                  value={draftDesc}
